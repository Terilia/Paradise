...
/datum/brain_trauma/special/obsessed/on_life()
	...
	if(viewing)
		...
		//Cut out the Mood change, Paradise doesn't have that system. 
		if(time_spent_away < 20 && next_warning > world.time)
			to_chat(src, "<span class='greentext'>The voices have released their hooks on my mind! I feel free again!</span>\n")
			next_warning = world.time + 300 //Avoid spamming
...
/datum/brain_trauma/special/obsessed/proc/out_of_view()
	time_spent_away += 20
	if(time_spent_away > 1800 && next_warning > world.time) //3 minutes
		//SEND_SIGNAL(owner, COMSIG_ADD_MOOD_EVENT, "creeping", /datum/mood_event/notcreepingsevere, obsession.name)
		//Cut out the Mood change, Paradise doesn't have that system. 
		to_chat(src, "<span class='boldwarning'>THEY NEEEEEEED OBSESSIONNNN!!</span>\n")
		next_warning = world.time + 300 
	else
		if(next_warning > world.time)
			next_warning = world.time + 300 
			//SEND_SIGNAL(owner, COMSIG_ADD_MOOD_EVENT, "creeping", /datum/mood_event/notcreeping, obsession.name)
			to_chat(src, "<span class='warning'>The voices are not happy, and they painfully contort my thoughts into getting back on task.</span>\n")
...
/datum/brain_trauma/special/obsessed/proc/find_obsession()
	...
	for(var/mob/Player in GLOB.player_list)//prevents crewmembers falling in love with nuke ops they never met, and other annoying hijinks
		if(Player.mind && Player.stat != DEAD && !isnewplayer(Player) && !isbrain(Player) && Player.client && Player != owner && !Player.offstation_role)
